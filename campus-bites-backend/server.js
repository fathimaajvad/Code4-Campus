...
// Add this line to create a route for the root URL ('/')
app.get('/', (req, res) => {
...
app.post('/api/create-order', async (req, res) => {
  const { amount, items, userId, userName, userRollNo, pickupTime, extraSuggestions } = req.body;
  try {
...
app.post('/api/admin/scan-qr', async (req, res) => {
  ...
  try {
    ...
    const isPremadeOnlyOrder = order.items.every(item =>
      item.category !== 'Staples' && item.category !== 'Curries' && item.category !== 'Meals'
    );
